...
function FormularioSolicitud() {
    ...
    // Track validation errors for sections
    ...
    // Track which section should be forcibly opened due to errors
    ...
    const validationSchema = Yup.object({
        ciSolicitante: Yup.number()
            .required('El C.I. es obligatorio'),
        nombreSolicitante: Yup.string()
            .required('El nombre es obligatorio'),
        apellidoSolicitante: Yup.string()
            .required('El apellido es obligatorio'),
        comunidad: Yup.string()
            .required('La comunidad es obligatoria'),
        fechaInicioIncendio: Yup.string()
            .required('La fecha de inicio es obligatoria'),
        listaProductos: Yup.string()
            .required('Ingrese la lista de productos'),
        listaProductosAPI: Yup.array()
    ...
    // Function to check if Solicitante section is complete
    ...
    // Function to check if Entrega section is complete
    ...
    // Function to check if Emergencia section is complete
    ...
    useEffect(() => {
        // Open section that needs attention if forced
        if (forceOpenSection) {
    ...
    let [showSolicitante, setShowSolicitante] = useState(false);
    ...
    const handleLocationChange = async (location) => {
        ...
        try {
            ...
            // Obtener provincia de manera más flexible
            // Para Bolivia, a veces la provincia está en state o en county
            let provincia = '';
            if (address.state && address.state.includes('Santa Cruz')) {
                // Si state contiene "Santa Cruz", probablemente sea el departamento
                // En este caso, la provincia podría estar en county
                provincia = address.county || '';
            } else {
                // De lo contrario, usamos state como provincia
                provincia = address.state || '';
            }
            
            // Building a much more detailed address string with all available components
            // pero excluyendo la provincia
            const addressComponents = [];
            
            // Add building name or house number if available
            ...
            // Create a detailed formatted address (sin provincia)
            const direccion = addressComponents.join(', ');
            
            console.log("Datos de dirección completos:", address);
            console.log("Provincia detectada:", provincia);
            console.log("Dirección sin provincia:", direccion);
            ...
            if (formikRef.current) {
                ...
                console.log("Formulario actualizado con provincia:", provincia);
            }
        } catch (error) {
    ...
    const handleSubmit = async (values, { setSubmitting, resetForm }) => {
        try {
            ...
            // If any section is incomplete, force open the first incomplete section
            ...
            console.log("Categoria value:", values.categoria);
            
            const categoria = values.categoria || "Incendio";
            console.log("Final categoria value:", categoria);
            
            // Format detailed location information
            let direccionFinal = ubicacion.direccion || values.direccion;
            
            // If we have raw address data, create a more precise address format
            ...
            // Reset section status after successful submission
            setSectionStatus({
                solicitante: false,
                entrega: false,
                emergencia: false
